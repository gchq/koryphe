...
/**
 * A composite {@link java.util.function.BiFunction} that applies each combiner in turn, supplying the result of each combiner as
...
public final class BiFunctionComposite<I, O> extends Composite<BiFunction> implements BiFunction<I, O, O> {
    ...
    public O apply(final I input, final O state) {
        Object result = state;
        for (final BiFunction function : getFunctions()) {
